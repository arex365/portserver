...
async function getCurrentPositions(symbol,index = 0) {
  ...
  try {
    ...
    const { headers, queryString } = getSignedHeader('GET', '/fapi/v2/account', {}, index);
    
    const resp = await axios.get(`${baseUrl}/fapi/v2/account?${queryString}`, { headers });
...
async function listPositions(index = 0) {
  ...
  try {
    ...
    const openPositions = resp.data.positions.filter(p => parseFloat(p.positionAmt) !== 0);
  ...
  try {
    ...
    for (const pos of positionsToClose) {
      ...
      try {
        ...
        const { headers: closeHeaders, queryString: closeQueryString } = getSignedHeader('POST', '/fapi/v1/order', closeParams);
        
        const closeResp = await axios.post(`${baseUrl}/fapi/v1/order?${closeQueryString}`, {}, { headers: closeHeaders });
